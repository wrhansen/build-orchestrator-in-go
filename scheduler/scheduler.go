...
func (r *RoundRobin) Score(t task.Task, nodes []*node.Node) map[string]float64 {
	nodeScores := make(map[string]float64) // hold the scores for each node
	var newWorker int                      // the worker that should be selected to run a task
	...
	for idx, node := range nodes { // iterate over nodes and assign score to each
		if idx == newWorker {
...
func (r *RoundRobin) Pick(scores map[string]float64, candidates []*node.Node) *node.Node {
	...
	for idx, node := range candidates { // iterate over slice of candidate nodes
		if idx == 0 { // handle special case of the first node in the slice
			bestNode = node
			lowestScore = scores[node.Name]
			continue
		}

		if scores[node.Name] < lowestScore { // check if node's score is lower than current lowest score
			bestNode = node
			lowestScore = scores[node.Name]
		}
	}
	return bestNode // returns node with the lowest score
}
...
// filter out nodes whose disk is less than the task's disk requirement
...
func (e *Epvm) Score(t task.Task, nodes []*node.Node) map[string]float64 {
	...
	for _, node := range nodes {
		...
		newMemPercent := (calculateLoad(memoryPercentAllocated+float64(t.Memory/1000), float64(node.Memory)))
